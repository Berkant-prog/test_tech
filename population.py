...
def read_file(filename):
    ...
    with open(filename, newline='', encoding='utf-8') as csvfile:
        ...
        # n-1 dictionnaires dont les clés sont les champs de la première ligne du fichier
...
def build_list_departements(data):
    ...
    departements = {
        (
            row["Code Officiel Département"],
            row["Nom Officiel Département"],
        )
        for row in data
        if row["Code Officiel Département"] and row["Nom Officiel Département"]
    }
...
def build_list_communes(data):
    ...
    communes = {
        (
            row["Code Officiel Commune / Arrondissement Municipal"],
            row["Nom Officiel Commune / Arrondissement Municipal"],
        )
        for row in data
    }
    l = sorted(communes, key=lambda commune: (commune[0], commune[1]))
    ...
    for row in data:
        ...
        if latest is None or year > latest[0]:
            latest = (
                year,
                row["Nom Officiel Commune / Arrondissement Municipal"],
                int(float(row["Population totale"])),
            )
...
def build_dict_departements(data):
    ...
    """
    # votre code ici
    communes = multi_dict(2, dict)
    for row in data:
        dpt = row["Code Officiel Département"]
        code_commune = row["Code Officiel Commune / Arrondissement Municipal"]
        nom_commune = row["Nom Officiel Commune / Arrondissement Municipal"]
        if not dpt or not code_commune or not nom_commune:
            continue

        commune = (code_commune, nom_commune)
        year = row["Année de recensement"]
        population = int(float(row["Population totale"]))
        if commune not in communes[dpt] or year > communes[dpt][commune][0]:
            communes[dpt][commune] = (year, population)

    return {
        dpt: (
            len(dpt_communes),
            sum(population for _, population in dpt_communes.values()),
        )
        for dpt, dpt_communes in communes.items()
    }
...
def main():
    ...
    departements = build_list_departements(data)
    assert len(departements) == 99
    assert departements[5] == ('06', 'Alpes-Maritimes')
    assert ('15', 'Cantal') in departements

    communes = build_list_communes(data)
    assert len(communes) == 35836
    assert communes[999] == ('02598', 'Pernant')

    assert get_pop_commune(data, '39124') == ('Chaumergy', 492, '2018')
    assert get_pop_commune(data, '00000') is None

    departement_stats = build_dict_departements(data)
    assert departement_stats['18'] == (290, 310910)
    assert departement_stats['75'] == (20, 2192485)

    assert stat_by_dpt(departement_stats, '77') == (514, 1433625)
    assert stat_by_dpt(departement_stats, 'Corse') is None
